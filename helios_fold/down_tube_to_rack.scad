...
tube_to_tube = 9;
rack = [1,-1,0]/sqrt(2) * (inner_r + rack_r + tube_to_tube);

d=0.05;

module rack(){
	translate(rack) rotate([0,-4,45]) //rotate([0,2.5,-90])
	...
	difference(){
		union(){
			hull(){ //this bit holds the rack tube
                ...
                translate([4,rack[1],0]) cube([d,-rack[1],h]);
			}
		}
	
		cylinder(r=inner_r+t/2,h=999,center=true); //clearance for frame tube 
        
        //clearance for nut/bolt
        translate([0,-inner_r-5,h/2]) cube([30,8,h-3],center=true);

		//clearance for rack
		hull() repeat([999,-999,0],2) rack();
		translate(rack) rotate(45) translate([0,-4-999,0]) cube([1,1,1]*999*2,center=true);
