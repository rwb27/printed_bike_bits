    ...
    difference(){
        union(){
            hull(){
                ...
                rotate([screw_angles[1],0,0]) reflect([0,0,1]) rotate([0,90,0]) rotate(screw_angles[0]) translate([bar_r+6,0,0]) cube([12,10,clip[0]],center=true);
            }
        ...
        rotate([screw_angles[1],0,0]) reflect([0,0,1]) rotate([0,90,0]) rotate(screw_angles[0]) translate([bar_r+6,0,0]){
            pinch_y(4,screw_l=8,gap=[30,2,20],extra_height=0.2,t=4);
...
module reinforcement(){
    difference(){
        translate([-pocket[0]/2+clip[0],bar_r+10,-5]) cube([pocket[0]-2*clip[0], bar_to_bag - 12*3 - clip[1] + 10, 10]);
        
        //tapped holes for mounting (M4)
        reflect([1,0,0]) translate([pocket[0]/2-clip[0],bar_r + 15, 0]) repeat([0,bar_to_bag - 12*3 - clip[1],0],2) rotate([0,90,0]){
            cylinder(r=4/2*0.95,h=30,center=true);
        }
        
        //cable tie/anchor point
        reflect([1,0,0]) translate([15,bar_r+18,0]) cube([5,5,999],center=true);
...
difference(){
    ...
    hull(){
                //fixing bolts
        rotate([screw_angles[1],0,0]) reflect([0,0,1]) rotate([0,90,0]) rotate(screw_angles[0]) translate([0,0,-99]){
            cube([999,d,999]);
        }
    }
}
